...
- **Deck PDF** is the canonical artifact submitted alongside this repo. `deck.md` is the Marp source if you want to re-render: `npx @marp-team/marp-cli@latest deck.md --pdf`. Figures referenced by Marp are regenerated by running the notebooks (outputs/figures/ is gitignored).
